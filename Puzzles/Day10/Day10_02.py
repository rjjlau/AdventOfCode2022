...
def roll_cycle(cycle, x, results):
    cycle += 1
    if max(cycle-1, 0) % 40 in range(x-1, x+2):
        results.append("#")
    else:
        results.append(".")
...
def answer(s):
    ...
    while len(commands) != 0:
        ...
        if command == "noop":
            cycle, results = roll_cycle(cycle, x, results)
        else:
            cycle, results = roll_cycle(cycle, x, results)
            cycle, results = roll_cycle(cycle, x, results)
            x += int(command.split()[1].strip())
    return "\n".join(["".join(results[i: i+40]) for i in range(0, len(results), 40)])
